...
function convertType(swaggerType, swagger) {
    ...
    if (swaggerType.hasOwnProperty('schema')) {
        ...
        if (swaggerType.minItems >= 0 && swaggerType.hasOwnProperty('title') && !swaggerType.$ref) {
            ...
            if (typespec.properties.length > 0) {
                typespec.tsType = `{ ${typespec.properties.map(p => `${p.name}:${p.tsType === 'ref' ? p.target : p.tsType}`).join(';')} }`;
            }
        }
    }
    ...
    typespec.isObject = typespec.tsType.substr(0,1) === '{';
    typespec.isArray = !!typespec.elementType;
